...
describe("DutchAuction – 03_commit", function () {
    it("should accept a valid commit during commit window", async function () {
        ...
        const hash = buildCommitHash(0n, qty, nonce);
        ...
        await expect(auction.connect(alice).commit(hash, [], { value: deposit }))
            ...
            .withArgs(await alice.getAddress(), hash, deposit, qty);
    });
    ...
    it("should store commit data and increment aggregates", async function () {
        ...
        expect(await auction.committedQty(await alice.getAddress())).to.equal(qty);
    ...
    it("should revert if msg.value is zero", async function () {
        ...
        const hash = buildCommitHash(0n, 10n, randomNonce());
    ...
    it("should revert if per-address cap exceeded", async function () {
        const ctx = await fixtureWithOverrides({ perAddressCap: 60n })();
        await time.increaseTo(ctx.startTime + 1n);
        ...
        const qty = 50n;
        const hash = buildCommitHash(0n, qty, randomNonce());
        await auction.connect(alice).commit(hash, [], { value: qty * priceTicks[0] });

        const secondQty = 20n;
        const secondHash = buildCommitHash(0n, secondQty, randomNonce());
        await expect(
            auction.connect(alice).commit(secondHash, [], { value: secondQty * priceTicks[0] })
    ...
    it("should allow commit when merkle root is empty", async function () {
        ...
        const qty = 30n;
        const hash = buildCommitHash(0n, qty, randomNonce());
        await expect(auction.connect(alice).commit(hash, [], { value: qty * priceTicks[0] })).to.not.be.reverted;
    });
    ...
    it("should accept a valid proof when merkle root is set", async function () {
        ...
        const hash = buildCommitHash(0n, qty, randomNonce());
        await expect(
            auction.connect(alice).commit(hash, proofFor(alice.address), { value: qty * priceTicks[0] })
        )
            .to.emit(auction, "CommitSubmitted")
            .withArgs(alice.address, hash, qty * priceTicks[0], qty);
    });
    ...
    it("should revert with InvalidProof when proof is incorrect", async function () {
        ...
        await expect(
            auction.connect(alice).commit(hash, wrongProof, { value: qty * priceTicks[0] })
    ...
    it("should allow a user to submit multiple commits", async function () {
        ...
        const qty1 = 20n;
        const qty2 = 30n;
        const deposit1 = qty1 * priceTicks[0];
        const deposit2 = qty2 * priceTicks[0];

        const hash1 = buildCommitHash(0n, qty1, randomNonce());
        const hash2 = buildCommitHash(0n, qty2, randomNonce());
